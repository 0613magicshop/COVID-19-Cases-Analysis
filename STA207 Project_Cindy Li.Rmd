...
Three-way ANOVA assumptions: Normality of data, homogeneity of variance, and indenpendence of observations.
...
### Model Assumptions & Parameters
Four-way ANOVA assumptions: Normality of data, homogeneity of variance, and indenpendence of observations.
...
## Three-Way ANOVA Model (1st Question of Interest)
### Normality
Recalled that the histogram generated from the original response variable 'New_cases' for the first question of interest was heavily right-tailed, a log transformation has been performed on the response variable. Also, the outliers were detected and removed within the exploratory data analysis section. Hence, the transformed and cleaned response variable's distribution satisfies the normality assumption for this ANOVA model and the histogram can be referred in the data transformation section. Also, as indicated by the QQ-plot, the tails are not heavy and the values distributed along the line, which means the data is approximately normally distributed.

### Independence
Based on the residuals vs. fitted values plot, it is reasonable to make the conclusion that the observations are independent since the expectations of residuals are around zero and there is no obvious pattern or trend for the values. Thus, the independence of observations assumption is met.

### Equal Variance 
The residuals vs. fitted values plot also indicated that the variances for error terms are mostly constant.
...
## Four-Way ANOVA Model (2nd Question of Interest)
### Normallity
Similarly, a log transformation has been done on the response variable 'Weekly_new_cases_per100000' since the histogram obtained for the untransformed data also had a heavy right tail. Based on the histogram generated for the transformed data, it's safe to conclude that the normality assumption for the second ANOVA model is also attained. There are a few outliers shown in the QQ-plot, however, the data follows a normal distribution overall.

### Independence
From the residuals vs fitted plot, the independence assumption is attained based on the fact that the expected values of residuals are around zero. Also, there is no obvious trend shown in the plot.

### Equal Variance
The homogeneity of variance assumption for the model is satisfied based on the residuals vs fitted values plot, with the fact that there is no significant difference in ranges of the variances.
...
Based on the outputs from the first ANOVA model, which answers the first question of interest on whether differences in WHO regions and the variants of COVID-19 (Delta and Omicron) have significant effects on the number of new cases. It seems that there are significant differences among number of COVID-10 new cases among these 7 different WHO regions. Among those WHO regions, AMRO and EURO seem to have the highest number of new cases. As for the effects of presences of certain variant on the new cases number, it is shown that both the presences of Delta and Omicron brought bursts of new COVID-19 cases. Also, the combinations of WHO regions versus both variants also have significant effects on the increases of cases. This indicates that, although the Delta and Omicron variants develop differently among WHO regions, they created a increasing trends in the number of new cases.

As for the second question of interest regarding the effects of vaccinations related factors on the recently reported new cases, some of the model results seem to be anti-intuitive. It is tested that only the total vaccinations per 100 population and the first vaccine date of the country, as well as the interaction of these two main effect, have significant influences on the newly reported cases from last week. The effects of different levels among these factor turn to be counter-intuitive: the higher the number of total vaccinations, the more cases reported; also, the later the first vaccinate date, the less new cases reported. The results for total vaccinations might be due to the fact that only fully vaccinated people have higher defensive abilities for the virus. However, vaccinations might make people loosen their alerts for the COVID, while they gathered too much while not having enough antibodies. It is reasonable that the later the first vaccinate date, the less new cases, since vaccinations are not as effective as the time vaccinated.

# Limitations & Recommendations
First of all, there are too many missing data or data with zeros in the original data retrieved from WHO. The model accuracy could be improved if there could be more completed data. Secondly, there are only two levels for each of the dummy variable used for the first model, which may reduce the accuracy of the model as well. In addition, the method for categorizing the variant time periods are not scientific enough, so there might be cavets in the associated outputs. Then, there are too many factors for the second ANOVA model, which may be improved by introducing blocking for ANOVA model.
